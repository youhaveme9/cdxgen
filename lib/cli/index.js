...
export async function createPythonBom(path, options) {
  ...
  if (pyProjectMode) {
    ...
    maturinMode = pyProjMap.maturinMode || false;
  ...
  if (maturinMode) {
    ...
    for (const f of cargoFiles) {
      if (DEBUG_MODE) {
        console.log(`Parsing ${f} (maturin Rust dependency)`);
      }
    ...
    for (const f of cargoLockFiles) {
      ...
      const cargoDeps = parseCargoDependencyData(cargoLockData);
      if (cargoDeps?.length) {
        dependencies = mergeDependencies(
          dependencies,
          cargoDeps,
