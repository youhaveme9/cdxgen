...
it("parse maturin pyproject.toml", () => {
  const retMap = parsePyProjectTomlFile("./test/data/pyproject_maturin.toml");
  assert.ok(retMap.maturinMode);
  assert.ok(!retMap.poetryMode);
  assert.ok(!retMap.uvMode);
  assert.ok(!retMap.hatchMode);
  assert.deepStrictEqual(retMap.parentComponent.name, "my-rust-python-lib");
  assert.deepStrictEqual(retMap.parentComponent.version, "0.3.0");
  assert.deepStrictEqual(
    retMap.parentComponent.description,
    "A Python library with Rust bindings using maturin and PyO3",
  );
  assert.deepStrictEqual(retMap.parentComponent.type, "application");
  assert.deepStrictEqual(
    retMap.parentComponent["bom-ref"],
    "pkg:pypi/my-rust-python-lib@0.3.0",
  );
  assert.deepStrictEqual(retMap.directDepsKeys, {
    numpy: true,
    "typing-extensions": true,
  });
  const nonMaturinMap = parsePyProjectTomlFile("./test/data/pyproject.toml");
  assert.ok(!nonMaturinMap.maturinMode);
});
